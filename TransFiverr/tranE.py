...
class TransE:
    def __init__(self, dbpath, entityList, relationList, tripleList, margin=1, learingRate=0.01, dim=10, L1=True):
        self.dbpath = dbpath
        self.margin = margin
    ...
    def transE(self, cI=20):
        ...
        for cycleIndex in range(cI):
            ...
            if cycleIndex % 100 == 0:
                ...
                self.writeRelationVector(self.dbpath + 'relationVector.txt')
                self.writeEntilyVector(self.dbpath + 'entityVector.txt')
                self.loss = 0
                ...
                if self.L1:
                    ...
                    for i in range(self.dim):  # 不知道有没有pythonic的写法（比如列表推倒或者numpy的函数）？
                        ...
                        if tempNegtative[i] >= 0:
                            ...
                            tempNegtativeL1.append(-1)
                    tempPositive = 2 * self.learingRate * array(tempPositiveL1)
                    tempNegtative = 2 * self.learingRate * array(tempNegtativeL1)
    ...
    def writeEntilyVector(self, dir):
        ...
        for entity in self.entityList.keys():
            data.append(entity + "\t")
            data.append(str(self.entityList[entity].tolist()))
            data.append("\n")
        entityVectorFile.writelines(data)
    ...
    def writeRelationVector(self, dir):
        ...
        for relation in self.relationList.keys():
            ...
            data.append(str(self.relationList[relation].tolist()))
            data.append("\n")
        relationVectorFile.writelines(data)
...
if __name__ == '__main__':
    ...
    dirEntity = dbpath + 'entity2id.txt'
    entityIdNum, entityList = openDetailsAndId(dirEntity)
    dirRelation = dbpath + 'relation2id.txt'
    relationIdNum, relationList = openDetailsAndId(dirRelation)
    dirTrain = dbpath + 'train.txt'
    tripleNum, tripleList = openTrain(dirTrain)
    print("打开TransE")
    transE = TransE(dbpath, entityList, relationList, tripleList, margin=1, learingRate=0.01, dim=100)
    print("TranE初始化")
    transE.initialize()
    transE.transE(15000)
